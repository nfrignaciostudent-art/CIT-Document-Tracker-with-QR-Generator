...
(function () {
  ...
  window.dashActions = function (d) {
    ...
    /* ── ADMIN ─────────────────────────────────────────────────────
       DESIGN RULE (Option B): Admin's ONLY workflow action is send_back.
       Release is triggered via the file-upload PATCH endpoint (not here).
       Reject belongs to Faculty — completely removed from admin.
    ───────────────────────────────────────────────────────────────── */
    if (role === 'admin') {
      ...
      /* Fallback for legacy/offline docs: check status directly.
         Only send_back is available — release/reject NOT shown as buttons. */
